...
void snail(int num, int (*ptr)[num])
{
	...
	while(value<=num*num){
		if(Hcount%2!=0){ // 가로이동이 홀수번째=우향일때 +1
			...
			tmpptr = tmpptr+limit-1+num; // 한칸 아래 포인터 전달
		}
		else{ // 가로이동이 짝수번째=좌향일때 -1
			for(i=0; i<limit; i++){
				*(tmpptr-i) = value++;
			}
			tmpptr = tmpptr-limit+1-num; // 한칸 위 포인터 전달
		}
		Hcount++;
		...
		if(Vcount%2!=0){ // 세로이동이 홀수번째=하향일떄 +num
			...
			tmpptr = tmpptr+num*(limit-1)-1; // 한칸 왼쪽 포인터 전달
		}
		else{ // 세로이동이 짝수번째=상향일때 -num
			for(i=0; i<limit; i++){
				*(tmpptr-(i*num)) = value++;
			}
			tmpptr = tmpptr-num*(limit-1)+1; // 한칸 오른쪽 포인터 전달
		}
		Vcount++;
	}
